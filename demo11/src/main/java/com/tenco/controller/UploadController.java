...
public class UploadController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		System.out.println("파일확인(파일이름) :" + filePart.getSubmittedFileName());
		
		// 스트림 준비 
		InputStream fileContent = filePart.getInputStream();
		// 출력 스트림 준비 -> 내 서버 컴퓨터에 File 객체 만들어서 저장함 계획
		...
		try {
			...
			// 6f058dc2-1090-4806-9005-3a064d77035a a.png
			String fileName = uuid + "_" + filePart.getSubmittedFileName();
			
			// 1단계 : 파일을 저장할 폴더를 미리 만들어보기
			// C 드라이버에 jsp_upload 폴더만들기 (직접만들어주세요)
